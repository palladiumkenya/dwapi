...
{
    public class PrepBehaviourRiskExtractRepository : BaseRepository<PrepBehaviourRiskExtract, Guid>, IPrepBehaviourRiskExtractRepository
    {
        public PrepBehaviourRiskExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<PrepBehaviourRiskExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.PrepBehaviourRiskExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<PrepBehaviourRiskExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(PrepBehaviourRiskExtract.Status)}=@Status,{nameof(PrepBehaviourRiskExtract.StatusDate)}=@StatusDate
                where
                    {nameof(PrepBehaviourRiskExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(PrepBehaviourRiskExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
