...
{
    [TestFixture]
    public class DiffOtzsLoaderTests
    {
      private IOtzLoader _loader;
        private IOtzSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IOtzLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IOtzSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(OtzExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.OtzExtracts.Count());
            Assert.False(_extractsContext.OtzExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.OtzExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.OtzExtracts.Count()}");
        }
    }
}
