...
{ 
    ...
    public class HtsClientsLinkageLoaderTests
    {
        ...
        public void should_Load_From_Temp_MsSQL()
        {
            ...
            var extract = TestInitializer.Iqtools.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientLinkage)));

            var loader = TestInitializer.ServiceProvider.GetService<IHtsClientsLinkageLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContext.HtsClientsLinkageExtracts.Any());
            Console.WriteLine($"extracted {_extractsContext.HtsClientsLinkageExtracts.Count()}");
        }

        [Test]
        public void should_Load_From_Temp_MySQL()
        {
            Assert.False(_extractsContextMySql.HtsClientsLinkageExtracts.Any());
            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientLinkage)));

            var loader = TestInitializer.ServiceProviderMysql.GetService<IHtsClientsLinkageLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContextMySql.HtsClientsLinkageExtracts.Any());
            Console.WriteLine($"extracted {_extractsContextMySql.HtsClientsLinkageExtracts.Count()}");
        }
    }
}
