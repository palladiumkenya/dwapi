...
{
    public class PatientMnchExtractRepository : BaseRepository<PatientMnchExtract, Guid>, IPatientMnchExtractRepository
    {
        public PatientMnchExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<PatientMnchExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.PatientMnchExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<PatientMnchExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(PatientMnchExtract.Status)}=@Status,{nameof(PatientMnchExtract.StatusDate)}=@StatusDate
                where
                    {nameof(PatientMnchExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(PatientMnchExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
