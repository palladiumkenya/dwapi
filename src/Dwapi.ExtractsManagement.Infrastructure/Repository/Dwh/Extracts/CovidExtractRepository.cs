...
{
    public class CovidExtractRepository : BaseRepository<CovidExtract, Guid>, ICovidExtractRepository
    {
        public CovidExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<CovidExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.CovidExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<CovidExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(CovidExtract.Status)}=@Status,{nameof(CovidExtract.StatusDate)}=@StatusDate
                where
                    {nameof(CovidExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(CovidExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
