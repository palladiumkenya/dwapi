...
{
    public class PrepExtractActivityNotificationHandler : IHandler<PrepExtractActivityNotification>
    {
        private IExtractHistoryRepository _extractHistoryRepository;

        public async Task Handle(PrepExtractActivityNotification domainEvent)
        {
            _extractHistoryRepository = Startup.ServiceProvider.GetService<IExtractHistoryRepository>();

            SaveExtractHistory(domainEvent);
            await Startup.PrepHubContext.Clients.All.SendAsync("ShowPrepProgress", domainEvent);
        }

        private void SaveExtractHistory(PrepExtractActivityNotification domainEvent)
        {
            int count = 0;
