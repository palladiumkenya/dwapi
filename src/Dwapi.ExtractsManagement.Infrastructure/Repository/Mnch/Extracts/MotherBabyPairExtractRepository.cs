...
{
    public class MotherBabyPairExtractRepository : BaseRepository<MotherBabyPairExtract, Guid>, IMotherBabyPairExtractRepository
    {
        public MotherBabyPairExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<MotherBabyPairExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.MotherBabyPairExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<MotherBabyPairExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(MotherBabyPairExtract.Status)}=@Status,{nameof(MotherBabyPairExtract.StatusDate)}=@StatusDate
                where
                    {nameof(MotherBabyPairExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(MotherBabyPairExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
