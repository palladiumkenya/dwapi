...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractPatientLaboratoryHandler :IRequestHandler<ExtractPatientLaboratory,bool>
    {
        ...
        public ExtractPatientLaboratoryHandler(IPatientLaboratorySourceExtractor patientLaboratorySourceExtractor, IExtractValidator extractValidator, IPatientLaboratoryLoader patientLaboratoryLoader, IClearDwhExtracts clearDwhExtracts, IExtractHistoryRepository extractHistoryRepository, IDiffLogRepository diffLogRepository,IIndicatorExtractRepository indicatorExtractRepository)
        {
        ...
        public async Task<bool> Handle(ExtractPatientLaboratory request, CancellationToken cancellationToken)
        {
            ...
            var difflog = _diffLogRepository.GetLog("NDWH", "PatientLaboratoryExtract",mflcode);
            ...
            if (request.DatabaseProtocol.SupportsDifferential)
            {
                if(null==difflog)
                    ...
                    if (true == loadChangesOnly)
                    {
                        ...
                        found = await _patientLaboratorySourceExtractor.Extract(request.Extract,
            ...
            //Load
            int loaded = await _patientLaboratoryLoader.Load(request.Extract.Id, found, request.DatabaseProtocol.SupportsDifferential);
