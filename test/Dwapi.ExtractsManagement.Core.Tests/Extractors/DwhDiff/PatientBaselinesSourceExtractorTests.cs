...
{
    [TestFixture]
    public class DiffPatientBaselinesSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempPatientBaselinesExtracts.Count());
            Assert.False(_extractsContext.TempPatientBaselinesExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempPatientBaselinesExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempPatientBaselinesExtracts.Count()}");
        }
    }
}
