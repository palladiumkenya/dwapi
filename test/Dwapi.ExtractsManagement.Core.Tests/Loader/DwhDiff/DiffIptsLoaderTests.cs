...
{
    [TestFixture]
    public class DiffIptsLoaderTests
    {
      private IIptLoader _loader;
        private IIptSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IIptLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IIptSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(IptExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.IptExtracts.Count());
            Assert.False(_extractsContext.IptExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.IptExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.IptExtracts.Count()}");
        }
    }
}
