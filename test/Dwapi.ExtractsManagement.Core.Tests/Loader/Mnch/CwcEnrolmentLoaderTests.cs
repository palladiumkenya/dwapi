...
{
    [TestFixture]
    public class CwcEnrolmentLoaderTests
    {
        private ICwcEnrolmentLoader _loader;
        private ICwcEnrolmentSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<ICwcEnrolmentLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<ICwcEnrolmentSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(CwcEnrolmentExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.CwcEnrolmentExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.CwcEnrolmentExtracts.Count()}");
        }
    }
}