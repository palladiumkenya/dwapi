...
{
    public class ArtFastTrackExtractRepository : BaseRepository<ArtFastTrackExtract, Guid>, IArtFastTrackExtractRepository
    {
        public ArtFastTrackExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<ArtFastTrackExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.ArtFastTrackExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<ArtFastTrackExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(ArtFastTrackExtract.Status)}=@Status,{nameof(ArtFastTrackExtract.StatusDate)}=@StatusDate
                where
                    {nameof(ArtFastTrackExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(ArtFastTrackExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
