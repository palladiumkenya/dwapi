...
{ 
    ...
    public class HtsClientTracingLoaderTests
    {
        ...
        public void should_Load_From_Temp_MsSQL()
        {
            ...
            var extract = TestInitializer.Iqtools.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientTracing)));

            var loader = TestInitializer.ServiceProvider.GetService<IHtsClientTracingLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContext.HtsClientTracingExtracts.Any());
            Console.WriteLine($"extracted {_extractsContext.HtsClientTracingExtracts.Count()}");
        }

        [Test]
        public void should_Load_From_Temp_MySQL()
        {
            Assert.False(_extractsContextMySql.HtsClientTracingExtracts.Any());
            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientTracing)));

            var loader = TestInitializer.ServiceProviderMysql.GetService<IHtsClientTracingLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContextMySql.HtsClientTracingExtracts.Any());
            Console.WriteLine($"extracted {_extractsContextMySql.HtsClientTracingExtracts.Count()}");
        }
    }
}
