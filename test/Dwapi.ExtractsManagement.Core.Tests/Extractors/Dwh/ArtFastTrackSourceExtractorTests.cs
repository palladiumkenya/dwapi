...
{
    [TestFixture]
    public class ArtFastTrackSourceExtractorTests
    {
        ...
        [TestCase(nameof(ArtFastTrackExtract))]
        public void should_Extract(string name)
        {
            Assert.False(_extractsContext.TempArtFastTrackExtracts.Any());
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempArtFastTrackExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempArtFastTrackExtracts.Count()}");
        }
    }
}
