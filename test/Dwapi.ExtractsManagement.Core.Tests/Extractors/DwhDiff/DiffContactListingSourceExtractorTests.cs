...
{
    [TestFixture]
    public class DiffContactListingSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempContactListingExtracts.Count());
            Assert.False(_extractsContext.TempContactListingExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempContactListingExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempContactListingExtracts.Count()}");
        }
    }
}
