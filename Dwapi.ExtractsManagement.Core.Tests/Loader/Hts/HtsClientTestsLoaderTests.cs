...
{ 
    ...
    public class HtsClientTestsLoaderTests
    {
        ...
        public void should_Load_From_Temp_MsSQL()
        {
            ...
            var extract = TestInitializer.Iqtools.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientTests)));

            var loader = TestInitializer.ServiceProvider.GetService<IHtsClientTestsLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContext.HtsClientTestsExtracts.Any());
            Console.WriteLine($"extracted {_extractsContext.HtsClientTestsExtracts.Count()}");
        }

        [Test]
        public void should_Load_From_Temp_MySQL()
        {
            Assert.False(_extractsContextMySql.HtsClientTestsExtracts.Any());
            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(HtsClientTests)));

            var loader = TestInitializer.ServiceProviderMysql.GetService<IHtsClientTestsLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContextMySql.HtsClientTestsExtracts.Any());
            Console.WriteLine($"extracted {_extractsContextMySql.HtsClientTestsExtracts.Count()}");
        }
    }
}
