...
{
    [TestFixture]
    public class HeiLoaderTests
    {
        private IHeiLoader _loader;
        private IHeiSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IHeiLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IHeiSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(HeiExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.HeiExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.HeiExtracts.Count()}");
        }
    }
}