...
{
    [TestFixture]
    public class PatientMnchLoaderTests
    {
        private IPatientMnchLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IPatientMnchLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IPatientMnchSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(PatientMnchExtract)));
        ...
        public void should_Load()
        {
            ...
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientMnchExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientMnchExtracts.Count()}");
        }
    }
}
