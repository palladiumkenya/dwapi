...
{
    [TestFixture]
    public class DiffPatientStatusLoaderTests
    {
     private IPatientStatusLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IPatientStatusLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IPatientStatusSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(PatientStatusExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientStatusExtracts.Count());
            Assert.False(_extractsContext.PatientStatusExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.PatientStatusExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientStatusExtracts.Count()}");
        }
    }
}
