...
{
    public class EnhancedAdherenceCounsellingSourceExtractor : IEnhancedAdherenceCounsellingSourceExtractor
    {
        private readonly IDwhExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempEnhancedAdherenceCounsellingExtractRepository _extractRepository;

        public EnhancedAdherenceCounsellingSourceExtractor(IDwhExtractSourceReader reader, IMediator mediator, ITempEnhancedAdherenceCounsellingExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempEnhancedAdherenceCounsellingExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord = Mapper.Map<IDataRecord, TempEnhancedAdherenceCounsellingExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new ExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempEnhancedAdherenceCounsellingExtract>();
                    }
                }
