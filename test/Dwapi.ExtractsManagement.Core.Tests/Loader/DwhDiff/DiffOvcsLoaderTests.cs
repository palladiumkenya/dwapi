...
{
    [TestFixture]
    public class DiffOvcsLoaderTests
    {
      private IOvcLoader _loader;
        private IOvcSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IOvcLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IOvcSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(OvcExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.OvcExtracts.Count());
            Assert.False(_extractsContext.OvcExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.OvcExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.OvcExtracts.Count()}");
        }
    }
}
