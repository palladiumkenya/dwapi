...
{ 
    ...
    public class HtsPartnerNotificationServicesLoaderTests
    {
        ...
        public void should_Load_From_Temp_MsSQL()
        {
            ...
            var extract = TestInitializer.Iqtools.Extracts.First(x => x.Name.IsSameAs(nameof(HtsPartnerNotificationServices)));

            var loader = TestInitializer.ServiceProvider.GetService<IHtsPartnerNotificationServicesLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContext.HtsPartnerNotificationServicesExtracts.Any());
            Console.WriteLine($"extracted {_extractsContext.HtsPartnerNotificationServicesExtracts.Count()}");
        }

        [Test]
        public void should_Load_From_Temp_MySQL()
        {
            Assert.False(_extractsContextMySql.HtsPartnerNotificationServicesExtracts.Any());
            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(HtsPartnerNotificationServices)));

            var loader = TestInitializer.ServiceProviderMysql.GetService<IHtsPartnerNotificationServicesLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContextMySql.HtsPartnerNotificationServicesExtracts.Any());
            Console.WriteLine($"extracted {_extractsContextMySql.HtsPartnerNotificationServicesExtracts.Count()}");
        }
    }
}
