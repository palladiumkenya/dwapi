...
{
    [TestFixture]
    public class MatVisitLoaderTests
    {
        private IMatVisitLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IMatVisitLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IMatVisitSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(MatVisitExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.MatVisitExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.MatVisitExtracts.Count()}");
        }
    }
}