...
{
    [TestFixture]
    public class DiffPatientArtLoaderTests
    {
        private IPatientArtLoader _loader;
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientArtExtracts.Count());
            Assert.False(_extractsContext.PatientArtExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.PatientArtExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientArtExtracts.Count()}");
        }
    }
}
