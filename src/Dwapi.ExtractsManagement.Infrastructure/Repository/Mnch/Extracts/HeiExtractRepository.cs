...
{
    public class HeiExtractRepository : BaseRepository<HeiExtract, Guid>, IHeiExtractRepository
    {
        public HeiExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<HeiExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.HeiExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<HeiExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(HeiExtract.Status)}=@Status,{nameof(HeiExtract.StatusDate)}=@StatusDate
                where
                    {nameof(HeiExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(HeiExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
