    ...
    public class PrepCareTerminationLoader : IPrepCareTerminationLoader
    {
        ...
        public async Task<int> Load(Guid extractId, int found, bool diffSupport)
        {
            ...
            try
            {
                ...
                int extractssitecode = (int) _tempPrepCareTerminationExtractRepository.GetSiteCode(query.ToString()).SiteCode;
                ...
                await _mediator.Publish(new DocketExtractLoaded("PREP", nameof(PrepCareTerminationExtract), extractssitecode));
