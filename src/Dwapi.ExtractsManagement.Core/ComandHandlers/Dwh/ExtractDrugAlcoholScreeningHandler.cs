...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractDrugAlcoholScreeningHandler :IRequestHandler<ExtractDrugAlcoholScreening,bool>
    {
        ...
        {
            //Extract
            int found = await _DrugAlcoholScreeningSourceExtractor.Extract(request.Extract, request.DatabaseProtocol);
            ...
            //Load
            int loaded = await _DrugAlcoholScreeningLoader.Load(request.Extract.Id, found);
