    ...
    public class PatientVisitExtractRepository : BaseRepository<PatientVisitExtract, Guid>, IPatientVisitExtractRepository
    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(PatientVisitExtract.Status)}=@Status,{nameof(PatientVisitExtract.StatusDate)}=@StatusDate
                where
                    {nameof(PatientVisitExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(PatientVisitExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
