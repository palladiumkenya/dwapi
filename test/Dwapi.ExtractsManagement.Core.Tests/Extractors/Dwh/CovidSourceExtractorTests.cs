...
{
    [TestFixture]
    public class CovidSourceExtractorTests
    {
        private ICovidSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _extractor = TestInitializer.ServiceProvider.GetService<ICovidSourceExtractor>();
        }

        [TestCase(nameof(CovidExtract))]
        public void should_Extract(string name)
        {
            Assert.False(_extractsContext.TempCovidExtracts.Any());
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempCovidExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempCovidExtracts.Count()}");
        }
    }
}
