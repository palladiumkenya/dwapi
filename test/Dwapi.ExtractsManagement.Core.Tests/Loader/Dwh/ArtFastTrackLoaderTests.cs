...
{
    [TestFixture]
    public class ArtFastTracksLoaderTests
    {
      private IArtFastTrackLoader _loader;
        private IArtFastTrackSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IArtFastTrackLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IArtFastTrackSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(ArtFastTrackExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.False(_extractsContext.ArtFastTrackExtracts.Any());

            var count = _loader.Load(_extract.Id,_count, false).Result;

            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.ArtFastTrackExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.ArtFastTrackExtracts.Count()}");
        }
    }
}
