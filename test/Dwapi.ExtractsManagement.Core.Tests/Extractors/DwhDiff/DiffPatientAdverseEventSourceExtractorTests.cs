...
{
    [TestFixture]
    public class DiffPatientAdverseEventSourceExtractorTests
    {
        private IPatientAdverseEventSourceExtractor _extractor;
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.False(_extractsContext.TempPatientAdverseEventExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempPatientAdverseEventExtracts.First().Date_Last_Modified.IsNullOrEmpty());
