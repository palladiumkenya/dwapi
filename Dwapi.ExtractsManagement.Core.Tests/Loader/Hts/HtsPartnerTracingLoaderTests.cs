...
namespace Dwapi.ExtractsManagement.Core.Tests.Loader.Hts
{
    ...
    {
        private IHtsPartnerTracingLoader _loader;
        private IHtsPartnerTracingSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IHtsPartnerTracingLoader>();
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.HtsPartnerTracingExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.HtsPartnerTracingExtracts.Count()}");
        }
    }
}
