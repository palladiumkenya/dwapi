...
{
    [TestFixture]
    public class HeiSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempHeiExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempHeiExtracts.Count()}");
        }
    }
}
