...
{
    [TestFixture]
    public class DiffGbvScreeningSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempGbvScreeningExtracts.Count());
            Assert.False(_extractsContext.TempGbvScreeningExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempGbvScreeningExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempGbvScreeningExtracts.Count()}");
        }
    }
}
