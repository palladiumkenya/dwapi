...
{
    [TestFixture]
    public class PncVisitLoaderTests
    {
        private IPncVisitLoader _loader;
        private IPncVisitSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IPncVisitLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IPncVisitSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(PncVisitExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.PncVisitExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.PncVisitExtracts.Count()}");
        }
    }
}