...
{
    [TestFixture]
    public class DiffDrugAlcoholScreeningsLoaderTests
    {
      private IDrugAlcoholScreeningLoader _loader;
        private IDrugAlcoholScreeningSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IDrugAlcoholScreeningLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IDrugAlcoholScreeningSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(DrugAlcoholScreeningExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.DrugAlcoholScreeningExtracts.Count());
            Assert.False(_extractsContext.DrugAlcoholScreeningExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.DrugAlcoholScreeningExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.DrugAlcoholScreeningExtracts.Count()}");
        }
    }
}
