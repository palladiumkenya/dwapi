...
{
    public class CwcEnrolmentExtractRepository : BaseRepository<CwcEnrolmentExtract, Guid>, ICwcEnrolmentExtractRepository
    {
        public CwcEnrolmentExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<CwcEnrolmentExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.CwcEnrolmentExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<CwcEnrolmentExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(CwcEnrolmentExtract.Status)}=@Status,{nameof(CwcEnrolmentExtract.StatusDate)}=@StatusDate
                where
                    {nameof(CwcEnrolmentExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(CwcEnrolmentExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
