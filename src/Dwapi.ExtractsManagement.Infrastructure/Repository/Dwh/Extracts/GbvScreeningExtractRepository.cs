...
{
    public class GbvScreeningExtractRepository : BaseRepository<GbvScreeningExtract, Guid>, IGbvScreeningExtractRepository
    {
        public GbvScreeningExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<GbvScreeningExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.GbvScreeningExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<GbvScreeningExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(GbvScreeningExtract.Status)}=@Status,{nameof(GbvScreeningExtract.StatusDate)}=@StatusDate
                where
                    {nameof(GbvScreeningExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(GbvScreeningExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
