...
{
    [TestFixture]
    public class MnchImmunizationLoaderTests
    {
      private IMnchImmunizationLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IMnchImmunizationLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IMnchImmunizationSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(MnchImmunizationExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.False(_extractsContext.MnchImmunizationExtracts.Any());

            var count = _loader.Load(_extract.Id,_count, false).Result;

            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.MnchImmunizationExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.MnchImmunizationExtracts.Count()}");
        }
    }
}
