...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Mnch
{
    public class ExtractCwcVisitHandler :IRequestHandler<ExtractCwcVisit,bool>
    {
        ...
        public ExtractCwcVisitHandler(ICwcVisitSourceExtractor cwcVisitSourceExtractor, IMnchExtractValidator extractValidator, ICwcVisitLoader cwcVisitLoader, IExtractHistoryRepository extractHistoryRepository, IDiffLogRepository diffLogRepository,IIndicatorExtractRepository indicatorExtractRepository)
        {
        ...
        public async Task<bool> Handle(ExtractCwcVisit request, CancellationToken cancellationToken)
        {
            ...
            var difflog = _diffLogRepository.GetLog("MNCH", "CwcVisitExtract", mflcode);
            var changesLoadedStatus= false;
            
            //Extract
            if (null == difflog)
            {
                found = await _cwcVisitSourceExtractor.Extract(request.Extract, request.DatabaseProtocol);
            }
            else
            {
                found  = await _cwcVisitSourceExtractor.Extract(request.Extract, request.DatabaseProtocol,difflog.MaxCreated,difflog.MaxModified,difflog.SiteCode);
            }
            //update status
            _diffLogRepository.UpdateExtractsSentStatus("MNCH", "CwcVisitExtract", changesLoadedStatus);
