...
{
    [TestFixture]
    public class MotherBabyPairSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempMotherBabyPairExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempMotherBabyPairExtracts.Count()}");
        }
    }
}
