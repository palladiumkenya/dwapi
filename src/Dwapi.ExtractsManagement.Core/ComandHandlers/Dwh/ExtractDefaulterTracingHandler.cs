...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractDefaulterTracingHandler :IRequestHandler<ExtractDefaulterTracing,bool>
    {
        ...
        public async Task<bool> Handle(ExtractDefaulterTracing request, CancellationToken cancellationToken)
        {
            ...
            var difflog = _diffLogRepository.GetLog("NDWH", "DefaulterTracingExtract", mflcode);
            ...
            if (request.DatabaseProtocol.SupportsDifferential)
            {
                if(null==difflog)
                    ...
                    if (true == loadChangesOnly)
                    {
                        ...
                        found = await _DefaulterTracingSourceExtractor.Extract(request.Extract,
            ...
            //update db status
            _diffLogRepository.UpdateExtractsSentStatus("NDWH", "DefaulterTracingExtract", changesLoadedStatus);
