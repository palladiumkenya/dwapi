...
{
    [TestFixture]
    public class DiffContactListingsLoaderTests
    {
      private IContactListingLoader _loader;
        private IContactListingSourceExtractor _extractor;
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.ContactListingExtracts.Count());
            Assert.False(_extractsContext.ContactListingExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.ContactListingExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.ContactListingExtracts.Count()}");
        }
    }
}
