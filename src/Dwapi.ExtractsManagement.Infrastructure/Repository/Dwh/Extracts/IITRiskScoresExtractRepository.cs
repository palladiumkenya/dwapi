...
{
    public class IITRiskScoresExtractRepository : BaseRepository<IITRiskScoresExtract, Guid>, IIITRiskScoresExtractRepository
    {
        public IITRiskScoresExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<IITRiskScoresExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.IITRiskScoresExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<IITRiskScoresExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(IITRiskScoresExtract.Status)}=@Status,{nameof(IITRiskScoresExtract.StatusDate)}=@StatusDate
                where
                    {nameof(IITRiskScoresExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(IITRiskScoresExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
