...
{
    [TestFixture]
    public class MotherBabyPairLoaderTests
    {
        private IMotherBabyPairLoader _loader;
        private IMotherBabyPairSourceExtractor _extractor;
        ...
        public void Init()
        {
            ...
            _extracts = TestInitializer.Extracts.Where(x => Extentions.IsSameAs(x.DocketId, "MNCH")).ToList();
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IMotherBabyPairLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IMotherBabyPairSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(MotherBabyPairExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.MotherBabyPairExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.MotherBabyPairExtracts.Count()}");
        }
    }
}