...
{
    [TestFixture]
    public class DiffDrugAlcoholScreeningSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempDrugAlcoholScreeningExtracts.Count());
            Assert.False(_extractsContext.TempDrugAlcoholScreeningExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempDrugAlcoholScreeningExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempDrugAlcoholScreeningExtracts.Count()}");
        }
    }
}
