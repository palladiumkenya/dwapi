...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractPatientPharmacyHandler :IRequestHandler<ExtractPatientPharmacy,bool>
    {
        ...
        public ExtractPatientPharmacyHandler(IPatientPharmacySourceExtractor patientPharmacySourceExtractor, IExtractValidator extractValidator, IPatientPharmacyLoader patientPharmacyLoader, IClearDwhExtracts clearDwhExtracts, IExtractHistoryRepository extractHistoryRepository, IDiffLogRepository diffLogRepository)
        {
        ...
        public async Task<bool> Handle(ExtractPatientPharmacy request, CancellationToken cancellationToken)
        {
            ...
            var difflog = _diffLogRepository.GetLog("NDWH", "PatientPharmacyExtract");
            ...
            {
                found  = await _patientPharmacySourceExtractor.Extract(request.Extract, request.DatabaseProtocol,difflog.MaxCreated,difflog.MaxModified,difflog.SiteCode);
            }
            //Extract
            ...
            //Load
            int loaded = await _patientPharmacyLoader.Load(request.Extract.Id, found, request.DatabaseProtocol.SupportsDifferential);
