...
{
    public class MnchImmunizationExtractRepository : BaseRepository<MnchImmunizationExtract, Guid>, IMnchImmunizationExtractRepository
    {
        public MnchImmunizationExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<MnchImmunizationExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.MnchImmunizationExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<MnchImmunizationExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(MnchImmunizationExtract.Status)}=@Status,{nameof(MnchImmunizationExtract.StatusDate)}=@StatusDate
                where
                    {nameof(MnchImmunizationExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(MnchImmunizationExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
