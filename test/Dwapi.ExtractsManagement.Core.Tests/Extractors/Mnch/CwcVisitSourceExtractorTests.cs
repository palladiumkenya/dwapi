...
{
    [TestFixture]
    public class CwcVisitSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempCwcVisitExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempCwcVisitExtracts.Count()}");
        }
    }
}
