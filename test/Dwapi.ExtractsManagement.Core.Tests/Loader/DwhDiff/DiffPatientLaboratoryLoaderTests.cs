...
{
    [TestFixture]
    public class DiffPatientLaboratoryLoaderTests
    {
       private IPatientLaboratoryLoader _loader;
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientLaboratoryExtracts.Count());
            Assert.False(_extractsContext.PatientLaboratoryExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.PatientLaboratoryExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientLaboratoryExtracts.Count()}");
        }
    }
}
