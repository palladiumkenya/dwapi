...
{
    [TestFixture]
    public class MnchEnrolmentLoaderTests
    {
        private IMnchEnrolmentLoader _loader;
        private IMnchEnrolmentSourceExtractor _extractor;
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.MnchEnrolmentExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.MnchEnrolmentExtracts.Count()}");
        }
    }
}