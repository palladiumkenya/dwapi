...
{
    [TestFixture]
    public class PatientSourceExtractorTests
    {
        ...
        public void should_Exract_From_Reader_MsSql()
        {
            ...
            var extractor = TestInitializer.ServiceProvider.GetService<IPatientSourceExtractor>();
        ...
        {
            Assert.Null(_extractsContextMySql.TempPatientExtracts.FirstOrDefault());

            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(PatientExtract)));

            var extractor = TestInitializer.ServiceProviderMysql.GetService<IPatientSourceExtractor>();
