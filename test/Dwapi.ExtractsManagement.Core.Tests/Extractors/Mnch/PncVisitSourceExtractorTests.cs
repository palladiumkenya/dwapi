...
{
    [TestFixture]
    public class PncVisitSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempPncVisitExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempPncVisitExtracts.Count()}");
        }
    }
}
