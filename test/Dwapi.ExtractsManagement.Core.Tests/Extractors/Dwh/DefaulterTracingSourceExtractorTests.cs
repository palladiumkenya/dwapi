...
{
    [TestFixture]
    public class DefaulterTracingSourceExtractorTests
    {
        private IDefaulterTracingSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _extractor = TestInitializer.ServiceProvider.GetService<IDefaulterTracingSourceExtractor>();
        }

        [TestCase(nameof(DefaulterTracingExtract))]
        public void should_Extract(string name)
        {
            Assert.False(_extractsContext.TempDefaulterTracingExtracts.Any());
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempDefaulterTracingExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempDefaulterTracingExtracts.Count()}");
        }
    }
}
