...
{
    [TestFixture]
    public class DiffAllergiesChronicIllnesssLoaderTests
    {
      private IAllergiesChronicIllnessLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IAllergiesChronicIllnessLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IAllergiesChronicIllnessSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(AllergiesChronicIllnessExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.AllergiesChronicIllnessExtracts.Count());
            Assert.False(_extractsContext.AllergiesChronicIllnessExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.AllergiesChronicIllnessExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.AllergiesChronicIllnessExtracts.Count()}");
        }
    }
}
