...
{
    [TestFixture]
    public class HtsEligibilityLoaderTests
    {
        private IHtsEligibilityExtractLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IHtsEligibilityExtractLoader>();
        ...
        public void should_Load()
        {
            ...
            Assert.False(_extractsContext.HtsEligibilityExtracts.Any());

            var count = _loader.Load(_extract.Id,_count, false).Result;

            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.HtsEligibilityExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.HtsEligibilityExtracts.Count()}");
        }
    }
}
