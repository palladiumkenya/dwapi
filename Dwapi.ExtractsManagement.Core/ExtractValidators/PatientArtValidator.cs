...
{
    public class PatientArtValidator : IValidator
    {
        private readonly IExtractUnitOfWork _unitOfWork;
        private string _extractName => nameof(TempPatientArtExtract);

        public PatientArtValidator(IExtractUnitOfWork unitOfWork)
        {
        ...
        public async Task Validate()
        {
            try
            {
                ...
                foreach (var validation in validations)
                {
                    ...
                    var list = await _unitOfWork.Context.Set<TempPatientArtExtract>().FromSql(commandText).ToListAsync();
