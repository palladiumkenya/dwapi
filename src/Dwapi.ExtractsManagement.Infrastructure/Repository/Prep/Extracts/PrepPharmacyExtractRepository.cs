...
{
    public class PrepPharmacyExtractRepository : BaseRepository<PrepPharmacyExtract, Guid>, IPrepPharmacyExtractRepository
    {
        public PrepPharmacyExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<PrepPharmacyExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.PrepPharmacyExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<PrepPharmacyExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(PrepPharmacyExtract.Status)}=@Status,{nameof(PrepPharmacyExtract.StatusDate)}=@StatusDate
                where
                    {nameof(PrepPharmacyExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(PrepPharmacyExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
