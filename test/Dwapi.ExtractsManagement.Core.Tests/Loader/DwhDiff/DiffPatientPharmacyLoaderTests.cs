...
{
    [TestFixture]
    public class DiffPatientPharmacyLoaderTests
    {
        private IPatientPharmacyLoader _loader;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IPatientPharmacyLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IPatientPharmacySourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(PatientPharmacyExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientPharmacyExtracts.Count());
            Assert.False(_extractsContext.PatientPharmacyExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.PatientPharmacyExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientPharmacyExtracts.Count()}");
        }
    }
}
