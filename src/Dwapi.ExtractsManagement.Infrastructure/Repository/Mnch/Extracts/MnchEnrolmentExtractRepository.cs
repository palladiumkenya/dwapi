...
{
    public class MnchEnrolmentExtractRepository : BaseRepository<MnchEnrolmentExtract, Guid>, IMnchEnrolmentExtractRepository
    {
        public MnchEnrolmentExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<MnchEnrolmentExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.MnchEnrolmentExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<MnchEnrolmentExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(MnchEnrolmentExtract.Status)}=@Status,{nameof(MnchEnrolmentExtract.StatusDate)}=@StatusDate
                where
                    {nameof(MnchEnrolmentExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(MnchEnrolmentExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
