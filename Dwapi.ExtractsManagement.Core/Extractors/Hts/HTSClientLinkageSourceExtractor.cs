...
namespace Dwapi.ExtractsManagement.Core.Extractors.Hts
{
    public class HTSClientLinkageSourceExtractor : IHTSClientLinkageSourceExtractor
    {
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            DomainEvents.Dispatch(new HtsNotification(new ExtractProgress(nameof(HTSClientLinkageExtract), "extracting...")));
            //DomainEvents.Dispatch(new CbsStatusNotification(extract.Id,ExtractStatus.Loading));

            var list = new List<TempHTSClientLinkageExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    count++;
                    // AutoMapper profiles
                    var extractRecord = Mapper.Map<IDataRecord, TempHTSClientLinkageExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        count = 0;
                        list =new List<TempHTSClientLinkageExtract>();
                    }
            ...
            try
            {
                ...
                DomainEvents.Dispatch(new HtsNotification(new ExtractProgress(nameof(HTSClientLinkageExtract), "extracted", totalCount, 0, 0, 0, 0)));
