...
{
    public class DrugAlcoholScreeningSourceExtractor : IDrugAlcoholScreeningSourceExtractor
    {
        private readonly IDwhExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempDrugAlcoholScreeningExtractRepository _extractRepository;

        public DrugAlcoholScreeningSourceExtractor(IDwhExtractSourceReader reader, IMediator mediator, ITempDrugAlcoholScreeningExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempDrugAlcoholScreeningExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord = Mapper.Map<IDataRecord, TempDrugAlcoholScreeningExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new ExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempDrugAlcoholScreeningExtract>();
                    }
                }
