...
{
    [TestFixture]
    public class DiffOvcSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempOvcExtracts.Count());
            Assert.False(_extractsContext.TempOvcExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempOvcExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempOvcExtracts.Count()}");
        }
    }
}
