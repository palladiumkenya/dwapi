...
{
    public class CwcEnrolmentSourceExtractor : ICwcEnrolmentSourceExtractor
    {
        private readonly IMnchExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempCwcEnrolmentExtractRepository _extractRepository;

        public CwcEnrolmentSourceExtractor(IMnchExtractSourceReader reader, IMediator mediator, ITempCwcEnrolmentExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempCwcEnrolmentExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord =   mapper.Map<IDataRecord, TempCwcEnrolmentExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new MnchExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempCwcEnrolmentExtract>();
                    }
                }
