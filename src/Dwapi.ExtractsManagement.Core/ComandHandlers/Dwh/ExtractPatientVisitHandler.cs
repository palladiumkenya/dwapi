...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractPatientVisitHandler :IRequestHandler<ExtractPatientVisit,bool>
    {
        ...
        public ExtractPatientVisitHandler(IPatientVisitSourceExtractor patientVisitSourceExtractor, IExtractValidator extractValidator, IPatientVisitLoader patientVisitLoader, IClearDwhExtracts clearDwhExtracts, IExtractHistoryRepository extractHistoryRepository)
        {
        ...
        public async Task<bool> Handle(ExtractPatientVisit request, CancellationToken cancellationToken)
        {
            ...
            //Load
            int loaded = await _patientVisitLoader.Load(request.Extract.Id, found, false);
