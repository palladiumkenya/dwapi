...
{
    [TestFixture]
    public class DiffPatientVisitSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempPatientVisitExtracts.Count());
            Assert.False(_extractsContext.TempPatientVisitExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempPatientVisitExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"extracted {_extractsContext.TempPatientVisitExtracts.Count()}");
        }
    }
}
