...
{
    [TestFixture]
    public class MnchArtLoaderTests
    {
        private IMnchArtLoader _loader;
        private IMnchArtSourceExtractor _extractor;
        ...
        public void should_Load()
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count, _extractsContext.MnchArtExtracts.Count());
            Log.Debug($"Temp {_count} Main {_extractsContext.MnchArtExtracts.Count()}");
        }
    }
}