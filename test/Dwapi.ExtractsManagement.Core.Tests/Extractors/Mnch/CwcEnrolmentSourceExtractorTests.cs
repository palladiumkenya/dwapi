...
{
    [TestFixture]
    public class CwcEnrolmentSourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempCwcEnrolmentExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempCwcEnrolmentExtracts.Count()}");
        }
    }
}
