...
{
    [TestFixture]
    public class IITRiskScoresSourceExtractorTests
    {
        ...
        [TestCase(nameof(IITRiskScoresExtract))]
        public void should_Extract(string name)
        {
            Assert.False(_extractsContext.TempIITRiskScoresExtracts.Any());
            ...
            Assert.True(count > 0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.TempIITRiskScoresExtracts.Count());
            Log.Debug($"extracted {_extractsContext.TempIITRiskScoresExtracts.Count()}");
        }
    }
}
