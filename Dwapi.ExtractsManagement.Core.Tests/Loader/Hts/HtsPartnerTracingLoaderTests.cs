...
{ 
    [TestFixture]
    public class HtsPartnerTracingLoaderTests
    {
        ...
        public void should_Load_From_Temp_MsSQL()
        {
            ...
            var extract = TestInitializer.Iqtools.Extracts.First(x => x.Name.IsSameAs(nameof(HtsPartnerTracing)));

            var loader = TestInitializer.ServiceProvider.GetService<IHtsPartnerTracingLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContext.HtsPartnerTracingExtracts.Any());
            Console.WriteLine($"extracted {_extractsContext.HtsPartnerTracingExtracts.Count()}");
        }

        [Test]
        public void should_Load_From_Temp_MySQL()
        {
            Assert.False(_extractsContextMySql.HtsPartnerTracingExtracts.Any());
            var extract = TestInitializer.KenyaEmr.Extracts.First(x => x.Name.IsSameAs(nameof(HtsPartnerTracing)));

            var loader = TestInitializer.ServiceProviderMysql.GetService<IHtsPartnerTracingLoader>();

            var loadCount = loader.Load(extract.Id, 0).Result;
            Assert.True(_extractsContextMySql.HtsPartnerTracingExtracts.Any());
            Console.WriteLine($"extracted {_extractsContextMySql.HtsPartnerTracingExtracts.Count()}");
        }
    }
}
