    ...
    public class DepressionScreeningLoader : IDepressionScreeningLoader
    {
        ...
        public async Task<int> Load(Guid extractId, int found, bool diffSupport)
        {
            ...
            try
            {
                ...
                int extractssitecode = (int) _tempDepressionScreeningExtractRepository.GetSiteCode(query.ToString()).SiteCode;
                ...
                await _mediator.Publish(new DocketExtractLoaded("NDWH", nameof(DepressionScreeningExtract), extractssitecode));
