...
{
    [TestFixture]
    public class DiffGbvScreeningsLoaderTests
    {
      private IGbvScreeningLoader _loader;
        private IGbvScreeningSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IGbvScreeningLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IGbvScreeningSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(GbvScreeningExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.GbvScreeningExtracts.Count());
            Assert.False(_extractsContext.GbvScreeningExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.GbvScreeningExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.GbvScreeningExtracts.Count()}");
        }
    }
}
