...
namespace Dwapi.ExtractsManagement.Core.Tests.Extractors.DwhDiff
{
    ...
    public class DiffPatientPharmacySourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.False(_extractsContext.TempPatientPharmacyExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempPatientPharmacyExtracts.First().Date_Last_Modified.IsNullOrEmpty());
