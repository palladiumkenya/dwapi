...
{
    [TestFixture]
    public class DiffPatientVisitsLoaderTests
    {
      private IPatientVisitLoader _loader;
        private IPatientVisitSourceExtractor _extractor;
        ...
        public void SetUp()
        {
            _loader = TestInitializer.ServiceProvider.GetService<IPatientVisitLoader>();
            _extractor = TestInitializer.ServiceProvider.GetService<IPatientVisitSourceExtractor>();
            _extract = _extracts.First(x => x.Name.IsSameAs(nameof(PatientVisitExtract)));
        ...
        public void should_Load()
        {
            ...
            Assert.True(count >0);
            _extractsContext = TestInitializer.ServiceProvider.GetService<ExtractsContext>();
            Assert.AreEqual(count,_extractsContext.PatientVisitExtracts.Count());
            Assert.False(_extractsContext.PatientVisitExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.PatientVisitExtracts.First().Date_Last_Modified.IsNullOrEmpty());
            Log.Debug($"Temp {_count} Main {_extractsContext.PatientVisitExtracts.Count()}");
        }
    }
}
