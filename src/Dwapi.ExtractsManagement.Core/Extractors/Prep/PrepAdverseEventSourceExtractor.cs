...
{
    public class PrepAdverseEventSourceExtractor : IPrepAdverseEventSourceExtractor
    {
        private readonly IPrepExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempPrepAdverseEventExtractRepository _extractRepository;

        public PrepAdverseEventSourceExtractor(IPrepExtractSourceReader reader, IMediator mediator, ITempPrepAdverseEventExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempPrepAdverseEventExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord =   mapper.Map<IDataRecord, TempPrepAdverseEventExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new PrepExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempPrepAdverseEventExtract>();
                    }
                }
