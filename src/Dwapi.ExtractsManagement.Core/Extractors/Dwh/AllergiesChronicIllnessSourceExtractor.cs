...
{
    public class AllergiesChronicIllnessSourceExtractor : IAllergiesChronicIllnessSourceExtractor
    {
        private readonly IDwhExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempAllergiesChronicIllnessExtractRepository _extractRepository;

        public AllergiesChronicIllnessSourceExtractor(IDwhExtractSourceReader reader, IMediator mediator, ITempAllergiesChronicIllnessExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempAllergiesChronicIllnessExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord = Mapper.Map<IDataRecord, TempAllergiesChronicIllnessExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new ExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempAllergiesChronicIllnessExtract>();
                    }
                }
