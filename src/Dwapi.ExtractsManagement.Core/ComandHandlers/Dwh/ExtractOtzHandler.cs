...
namespace Dwapi.ExtractsManagement.Core.ComandHandlers.Dwh
{
    public class ExtractOtzHandler :IRequestHandler<ExtractOtz,bool>
    {
        ...
        public ExtractOtzHandler(IOtzSourceExtractor OtzSourceExtractor, IExtractValidator extractValidator, IOtzLoader OtzLoader, IClearDwhExtracts clearDwhExtracts, IExtractHistoryRepository extractHistoryRepository, IDiffLogRepository diffLogRepository,IIndicatorExtractRepository indicatorExtractRepository)
        {
        ...
        public async Task<bool> Handle(ExtractOtz request, CancellationToken cancellationToken)
        {
            ...
            var difflog = _diffLogRepository.GetLog("NDWH", "OtzExtract",mflcode);
            ...
            if (request.DatabaseProtocol.SupportsDifferential)
            {
                if(null==difflog)
                    ...
                    if (true == loadChangesOnly)
                    {
                        ...
                        found = await _OtzSourceExtractor.Extract(request.Extract,
            ...
            //Extract
            _diffLogRepository.UpdateExtractsSentStatus("NDWH", "OtzExtract", changesLoadedStatus);
