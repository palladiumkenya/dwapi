...
{
    public class PrepLabExtractRepository : BaseRepository<PrepLabExtract, Guid>, IPrepLabExtractRepository
    {
        public PrepLabExtractRepository(ExtractsContext context) : base(context)
        {
        }

        public bool BatchInsert(IEnumerable<PrepLabExtract> extracts)
        {
        ...
        {
            var sql = $"SELECT * FROM {nameof(ExtractsContext.PrepLabExtracts)} Where Id IN @Ids";
            var ids = sentItems.Select(x => x.Id).ToArray();

            using (var cn = GetNewConnection())
            {

                var mpi = cn.Query<PrepLabExtract>(sql, new { Ids = ids }).ToList()
                    .Select(x =>
                    {
        ...
        public long UpdateDiffSendStatus()
        {
            ...
            using (var cn = GetNewConnection())
            {
                ...
                SET
                    {nameof(PrepLabExtract.Status)}=@Status,{nameof(PrepLabExtract.StatusDate)}=@StatusDate
                where
                    {nameof(PrepLabExtract.PatientPK)} in (select PatientPK from {nameof(ExtractsContext.PatientExtracts)}) AND
                    {nameof(PrepLabExtract.SiteCode)} in (select SiteCode from {nameof(ExtractsContext.PatientExtracts)})
                ";
