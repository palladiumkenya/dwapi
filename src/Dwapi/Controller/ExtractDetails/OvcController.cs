using Dwapi.ExtractsManagement.Core.Interfaces.Repository.Dwh;
using Microsoft.AspNetCore.Mvc;
using Serilog;
using System;
using System.Linq;
using System.Threading.Tasks;

namespace Dwapi.Controller.ExtractDetails
{
    [Produces("application/json")]
    [Route("api/Ovc")]
    public class OvcController : Microsoft.AspNetCore.Mvc.Controller
    {
        private readonly ITempOvcExtractRepository _tempOvcExtractRepository;
        private readonly IOvcExtractRepository _ovcExtractRepository;
        private readonly ITempOvcExtractErrorSummaryRepository _errorSummaryRepository;

        public OvcController(ITempOvcExtractRepository tempOvcExtractRepository, IOvcExtractRepository OvcExtractRepository, ITempOvcExtractErrorSummaryRepository errorSummaryRepository)
        {
            _tempOvcExtractRepository = tempOvcExtractRepository;
            _ovcExtractRepository = OvcExtractRepository;
            _errorSummaryRepository = errorSummaryRepository;
        }

        [HttpGet("ValidCount")]
        public async Task<IActionResult> GetValidCount()
        {
            try
            {
                var count = await _ovcExtractRepository.GetCount();
                return Ok(count);
            }
            catch (Exception e)
            {
                var msg = $"Error loading valid Patient Extracts";
                Log.Error(msg);
                Log.Error($"{e}");
                return StatusCode(500, msg);
            }
        }

        [HttpGet("LoadValid/{page}/{pageSize}")]
        public async Task<IActionResult> LoadValid(int? page,int pageSize)
        {
            try
            {
                var tempOvcExtracts = await _ovcExtractRepository.GetAll(page,pageSize);
                return Ok(tempOvcExtracts.ToList());
            }
            catch (Exception e)
            {
                var msg = $"Error loading valid Ovc Extracts";
                Log.Error(msg);
                Log.Error($"{e}");
                return StatusCode(500, msg);
            }
        }

        [HttpGet("LoadErrors")]
        public IActionResult LoadErrors()
        {
            try
            {
                var tempOvcExtracts = _tempOvcExtractRepository.GetAll().Where(n => n.CheckError).ToList();
                return Ok(tempOvcExtracts);
            }
            catch (Exception e)
            {
                var msg = $"Error loading Ovc Extracts with errors";
                Log.Error(msg);
                Log.Error($"{e}");
                return StatusCode(500, msg);
            }
        }

        [HttpGet("LoadValidations")]
        public IActionResult LoadValidations()
        {
            try
            {
                var errorSummary = _errorSummaryRepository.GetAll().OrderByDescending(x=>x.Type).ToList();
                return Ok(errorSummary);
            }
            catch (Exception e)
            {
                var msg = $"Error loading Patient Status error summary";
                Log.Error(msg);
                Log.Error($"{e}");
                return StatusCode(500, msg);
            }
        }
    }
}
