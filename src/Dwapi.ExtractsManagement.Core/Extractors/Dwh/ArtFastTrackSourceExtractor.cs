...
{
    public class ArtFastTrackSourceExtractor : IArtFastTrackSourceExtractor
    {
        private readonly IDwhExtractSourceReader _reader;
        private readonly IMediator _mediator;
        private readonly ITempArtFastTrackExtractRepository _extractRepository;

        public ArtFastTrackSourceExtractor(IDwhExtractSourceReader reader, IMediator mediator, ITempArtFastTrackExtractRepository extractRepository)
        {
            _reader = reader;
            _mediator = mediator;
        ...
        public async Task<int> Extract(DbExtract extract, DbProtocol dbProtocol)
        {
            ...
            var list = new List<TempArtFastTrackExtract>();
            ...
            using (var rdr = await _reader.ExecuteReader(dbProtocol, extract))
            {
                while (rdr.Read())
                {
                    ...
                    loaded++;
                    // AutoMapper profiles
                    var extractRecord =   mapper.Map<IDataRecord, TempArtFastTrackExtract>(rdr);
                    ...
                    if (count == batch)
                    {
                        ...
                        DomainEvents.Dispatch(
                            new ExtractActivityNotification(extract.Id, new DwhProgress(
                                ...
                                loaded, 0, 0, 0, 0)));
                        list = new List<TempArtFastTrackExtract>();
                    }
                }
