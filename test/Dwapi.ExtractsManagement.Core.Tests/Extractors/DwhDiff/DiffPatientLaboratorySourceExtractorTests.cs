...
namespace Dwapi.ExtractsManagement.Core.Tests.Extractors.DwhDiff
{
    ...
    public class DiffPatientLaboratorySourceExtractorTests
    {
        ...
        public void should_Extract(string name)
        {
            ...
            Assert.False(_extractsContext.TempPatientLaboratoryExtracts.First().Date_Created.IsNullOrEmpty());
            Assert.False(_extractsContext.TempPatientLaboratoryExtracts.First().Date_Last_Modified.IsNullOrEmpty());
